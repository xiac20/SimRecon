...
# TODO: CHANGE THIS PATH ACCORDING TO YOUR FILE STRUCTURE
...
def space_aware_frame_sampling(vggt, images, K, dtype):
    ...
    """
    with torch.no_grad():
        with torch.amp.autocast(device_type="cuda", dtype=dtype), sdpa_kernel(SDPBackend.EFFICIENT_ATTENTION):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_gpus", type=int, default=None, help="Number of GPUs to use for parallel processing. Defaults to all available GPUs.")
    args = parser.parse_args()

    available_gpus = torch.cuda.device_count()
    if available_gpus == 0:
        raise RuntimeError("No CUDA devices available for processing.")
    ...
    num_gpus = min(num_gpus, len(all_videos))
    video_splits = [list(split) for split in np.array_split(all_videos, num_gpus) if len(split) > 0]

    processes = []
    for device_id, video_subset in enumerate(video_splits):
